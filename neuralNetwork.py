...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size):
        # Initialize the neural network with random weights and biases
         # We model a neural network with 1 hidden layer, with x input connections and y output connection.
        np.random.seed(1)
        # We assign random weights to a input_size x hidden_size matrix, with values in the range -1 to 1
        # and mean 0.
    ...
    def train(self, input_data, target, episodes, learning_rate):
        # Training loop
        for episod in range(episodes):
            ...
            if episod % 100 == 0:
                print(f"Episod {episod}: Loss {np.mean(np.square(target - output))}")
...
# Training loop
q_network.train(input_data, output_y, episodes=10000, learning_rate=0.1)
